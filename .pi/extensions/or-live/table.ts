 ...
 *
 * Uses cli-table3's built-in style system for coloring:
 * - Header rows: yellow
 * - Border chars: grey (dim but visible on both light/dark terminals)
 *
 ...
 *
 * Coloring is applied via cli-table3's built-in style system:
 * - `head: ["yellow"]` — yellow header labels
 * - `border: ["grey"]` — grey border characters
 */
export function renderTable<T>(
  ...
  // Strip ANSI codes to get the visible width of the rendered table.
  const stripAnsi = (s: string) => s.replace(/\x1b\[[0-9;]*m/g, "");

  // Build a titled top line and splice it in place of the plain border.
  if (titlePrefix || title) {
    const lines = output.split("\n");
    const visibleWidth = stripAnsi(lines[0]).length;
    let titleLine: string;
    if (titlePrefix) {
      const fill = "─".repeat(Math.max(0, visibleWidth - titlePrefix.length - 2));
      titleLine = `┌${titlePrefix}${fill}┐`;
    } else {
      const fill = "─".repeat(Math.max(0, visibleWidth - title.length - 4));
      titleLine = `┌─ ${title} ─${fill}┐`;
    }
    output = [titleLine, ...lines.slice(1)].join("\n");
  }
